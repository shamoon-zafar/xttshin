...
from TTS.tts.layers.vits2.duration_discriminator import DurationDiscriminatorV2
from TTS.tts.layers.vits2.discriminator import Vits2Discriminator
...
class Vits2Args(Coqpit):
    ...
    Args:
        ...
        periods_multi_period_discriminator (List[int]):
            Periods values for Vits2 Multi-Period Discriminator. Defaults to `[2, 3, 5, 7, 11]`.
    ...
    use_transformer_flow_layer: bool = True
...
class Vits2(BaseTTS):
    ...
    def forward_mas(self, outputs, z_p, m_p, logs_p, x, x_mask, y_mask, g, lang_emb):
        ...
        with torch.no_grad():
            ...
            # Using noise scaled MAS
            if self.args.use_noise_scaled_mas:
                epsilon = (
                    torch.std(logp)
        ...
        if self.args.use_sdp:
            ...
            logw_ = torch.log(attn_durations + 1e-6) * x_mask
        ...
        return outputs, attn, logw_ if self.args.use_sdp else attn_log_durations , logw if self.args.use_sdp else log_durations
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        # duration predictor
        outputs, attn, logw_ , logw = self.forward_mas(outputs, z_p, m_p, logs_p, x, x_mask, y_mask, g=g, lang_emb=lang_emb)
        # expand prior
        ...
        outputs.update(
            {
                ...
                "real_durations": logw_,
                "predicted_durations": logw,
            }
        )
        return outputs
    ...
    def train_step(self, batch: dict, criterion: nn.Module, optimizer_idx: int) -> Tuple[Dict, Dict]:
        ...
        if optimizer_idx == 0:
            ...
            scores_disc_fake, _, scores_disc_real, _ = self.disc(
                self.model_outputs_cache["model_outputs"].detach(), self.model_outputs_cache["waveform_seg"]
            )
    ...
    def get_scheduler(self, optimizer) -> List:
        ...
        scheduler_DUR = get_scheduler(self.config.lr_scheduler_gen, self.config.lr_scheduler_gen_params, optimizer[2])
        return [scheduler_D, scheduler_G, scheduler_DUR]
    ...
    def on_train_step_start(self, trainer):
        """MAS noise scale."""
        current_mas_noise_scale = self.args.mas_noise_scale_initial - self.args.noise_scale_delta * trainer.epochs_done
    ...
    #     from NEWTTS.tts.utils.text.cleaners import basic_cleaners
    ...
    #     new_chk = rehash_fairseq_vits2_checkpoint(checkpoint_file)
    ...
    @staticmethod
    def init_from_config(config: "Vits2Config", samples: Union[List[List], List[Dict]] = None):
        """Initiate model from config

        Args:
            config (Vits2Config): Model config.
    ...
    def export_onnx(self, output_path: str = "coqui_vits2.onnx", verbose: bool = True):
